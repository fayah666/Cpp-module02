...
- [III. New rules](#chapter-iii-new-rules)
- [IV. AI Instructions](#chapter-iv-ai-instructions)
- [V. Exercise 00: My First Class in Orthodox Canonical Form](#chapter-v-exercise-00-my-first-class-in-orthodox-canonical-form)
- [VI. Exercise 01: Towards a more useful fixed-point number class](#chapter-vi-exercise-01-towards-a-more-useful-fixed-point-number-class)
- [VII. Exercise 02: Now we’re talking](#chapter-vii-exercise-02-now-were-talking)
- [VIII. Exercise 03: BSP](#chapter-viii-exercise-03-bsp)
...
The goal of these modules is to introduce you to Object-Oriented Programming. This will be the starting point of your C++ journey. Many languages are recommended for learning OOP, but we decided to choose C++ since it’s derived from your old friend C.

Because this is a complex language, and in order to keep things simple, your code will comply with the **C++98 standard**. We are aware that modern C++ is quite different in many aspects. So, if you want to become a proficient C++ developer, it is up to you to go further after the 42 Common Core!
...
> **Note:** Regarding the Makefile for C++ projects, the same rules as in C apply (see the Norm chapter about the Makefile).
>
...
> **Important:** Split your class code into two files. The header file (`.hpp`/`.h`) contains the class definition, whereas the source file (`.cpp`) contains the implementation.
...
### Operator Overload:
- Add the following function to the `Fixed` class files: An overload of the insertion (`<<`) operator that inserts a floating-point representation of the fixed-point number into the output stream object passed as a parameter.
...
- **The 4 increment/decrement operators:** (pre-increment and post-increment, pre-decrement and post-decrement), which will increase or decrease the fixed-point value by the smallest representable `ϵ`, such that `1 + ϵ > 1`.
...
> ⚠️ **Warning:** If you ever do a division by 0, it is acceptable that the program crashes.
...
> **Note:** BSP stands for Binary Space Partitioning. You are welcome. :)
...
> ⚠️ **Important:** Thus, if the point is a vertex or on an edge, it will return `False`.
...
- Modify a display.
- Adjust a data structure to store new information, etc.
